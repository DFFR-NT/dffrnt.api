	...
	// COMPONENTS ----------------------------------------------------------
		...
		// DRAFT ///////////////////////////////////////////////////////////
			...
			Draft.Field 		= CRC({
				...
				numTyps: ['date','datetime-local','number','radio','checkbox'],
				txtTyps: [
					'text','textarea','password','color','email','month','select',
					'range','search','tel','time','url','week','list'
				],
				isType:  	function (type, types) { 
					let typs = this[types];
					return 	function (param, kind) {
						// ----------------------------------------------------
						if (!!param.type&&param.type.hasOwnProperty(type)) {
							return typs.has(kind);
						} else return false;
					}
				},
				getType:  	 function (type) {
					...
					if (!!type) {
						if (IS(type)=='object') {
							return Object.keys(type)[0].toLowerCase();
				...
				render: 	 function () {
					let props = Object.assign({ matches: {} }, this.props),
						...
						isTxA = kind == 'textarea',
						isSlc = kind == 'select',
						isOpt = ['checkbox','radio'].has(kind),
						isMlt = isSlc||isOpt,
						isNum = isTyp('Number','numTyps')(props,kind),
						isLst = isTyp(  'List','txtTyps')(props,kind)&&!isMlt,
						input = Assign({
							name: name, id: fid, 
								...
								{ isLst ?
									...
									( isMlt ? 
										( isOpt ? 
											...
											<textarea key={name} ref='field' rows="1" {...input} /> :

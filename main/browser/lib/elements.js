...
module.exports = function (global) {
	...
	var Access = IO('/api-accessor');
	var Socket = IO(NMESPC);
	var IOs = { Access: Access, Socket: Socket };

	var Actions = require('./actions.js')(Reflux);
	var Spaces = require('./spaces');
	var Stores = require('./stores.js')(Reflux, Actions, Spaces, IOs);
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					'div',
					null,
					'Your session expired. Gotta login again... :/'
				),
				React.createElement(
					'div',
					null,
					'The connection was Lost! :s'
				)
			);
		}
	});
	...
	var Stat = CRC({
		...
		render: function render() {
			...
			return React.createElement('var', { id: 'stat', 'data-value': status });
		}
	});
